...
def draw_squares_from_circle(n, circle, window):
    ...
    Q.attach_to(window)
    for k in range(n):
        square = rg.Square(rg.Point(Q.center.x + k * Q.radius, Q.center.y + k * Q.radius), 2 * Q.radius)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        line = rg.Line(rg.Point(rectangle1.get_center().x - .5 * k * rectangle1.get_width(), rectangle1.get_center().y + .5 * k * rectangle1.get_height()),rg.Point(rectangle2.get_center().x - .5 * k * rectangle1.get_width(), rectangle2.get_center().y + .5 * k * rectangle1.get_height()))
        line.thickness = 5
        if k % 2 == 0:
